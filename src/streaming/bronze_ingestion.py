...
class BronzeIngestion:
    ...
    def _create_spark_session(self) -> SparkSession:
        ...
        return (
            ...
            .appName("LogisticsBronzeIngestion")
    ...
    def _validate_coordinates(self, df: DataFrame, lat_col: str = "latitude", lng_col: str = "longitude") -> DataFrame:
        """Validate coordinates are within India bounds."""
        return df.withColumn(
            "is_valid_location",
            (F.col(lat_col).between(8.0, 37.0)) &
            (F.col(lng_col).between(68.0, 97.5))
        )
    ...
    def ingest_vehicle_positions(self) -> None:
        ...
        raw_df = self._read_kafka_stream("vehicle_positions")
        parsed_df = self._parse_json(raw_df, SCHEMAS["vehicle_positions"])

        # Add validation columns
        validated_df = (
            self._validate_coordinates(parsed_df)
            .withColumn(
                "is_valid_speed",
                F.col("speed_kmh").between(0, 200) | F.col("speed_kmh").isNull()
            )
        )

        query = self._write_to_delta(validated_df, "vehicle_positions")
    ...
    def ingest_vehicle_telemetry(self) -> None:
        ...
        raw_df = self._read_kafka_stream("vehicle_telemetry")
        parsed_df = self._parse_json(raw_df, SCHEMAS["vehicle_telemetry"])

        query = self._write_to_delta(parsed_df, "vehicle_telemetry")
    ...
    def ingest_shipment_events(self) -> None:
        ...
        raw_df = self._read_kafka_stream("shipment_events")
        parsed_df = self._parse_json(raw_df, SCHEMAS["shipment_events"])
        ...
        query = self._write_to_delta(validated_df, "shipment_events")
    ...
    def ingest_agent_positions(self) -> None:
        ...
        raw_df = self._read_kafka_stream("agent_positions")
        parsed_df = self._parse_json(raw_df, SCHEMAS["agent_positions"])

        validated_df = self._validate_coordinates(parsed_df)

        query = self._write_to_delta(validated_df, "agent_positions")
    ...
    def ingest_delivery_events(self) -> None:
        ...
        raw_df = self._read_kafka_stream("delivery_events")
        parsed_df = self._parse_json(raw_df, SCHEMAS["delivery_events"])

        # Validate delivery locations
        validated_df = self._validate_coordinates(
            parsed_df,
            lat_col="delivery_lat",
            lng_col="delivery_lng"
        )

        query = self._write_to_delta(validated_df, "delivery_events")
    ...
    def ingest_alerts(self) -> None:
        ...
        raw_df = self._read_kafka_stream("alerts")
        parsed_df = self._parse_json(raw_df, SCHEMAS["alerts"])

        query = self._write_to_delta(parsed_df, "alerts")
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=["all"],
        choices=["all", "vehicle_positions", "vehicle_telemetry", "shipment_events",
                 "agent_positions", "delivery_events", "alerts"],
